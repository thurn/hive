...
thread="${CODEX_THREAD_ID:?}" && hive-bd --actor "$thread" create --title 'Scoped work' --description '...' --acceptance '...' --priority 2 --metadata "{\"hive_project\":\"example\",\"hive_origin_thread\":\"$thread\"}" --json
```
