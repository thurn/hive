...
### Dashboard server and UI builds

Run `hive dashboard serve` to bind `127.0.0.1:4320`; `--port PORT` selects another loopback port. Stop it with Ctrl-C or terminate that process. A separate `telemetry watch` collector supplies observations. The server rejects foreign Host headers, cross-site browser requests and methods other than GET/HEAD; it exposes no CORS permission. It has no task-writing endpoints. Requests without browser fetch metadata are permitted for local tools.

Each API request selects current local master. UI output is cached by the Git `dashboard` tree, so Python-only promotions reuse the bundle and UI promotions build on the next page load without restarting. The resident routing/static module needs a restart if changed. Install the exact Node version in `dashboard/.nvmrc`; first-time dependencies need network access for `npm ci --ignore-scripts`. Builds run TypeScript and Vite in a temporary source copy and keep source snapshots untouched. Code failures wait for a new UI tree; environment failures retry after an increasing delay (one minute to one hour). Logs and markers are under `dashboard-builds/` in local state. `hive dashboard build FULL_COMMIT_HASH --retry` clears the marker and retries explicitly. A failed build serves the last successful UI with a visible warning when one exists.

After successful builds, the newest five bundles and bundles used within 24 hours remain, along with their dependency caches. Old code-failure markers expire after seven days. To reset all UI caches, stop the dashboard process and remove `dashboard-builds/` and `dashboard-node/` beneath the configured local state directory, then restart. Full delivery validation now has a 240-second limit to accommodate the HTTP timeout/concurrency journeys; the fast gate remains 30 seconds.
