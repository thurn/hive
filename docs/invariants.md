...
- Observation derives thread IDs only from bead creator metadata, native assignee fields, and bead ownership-interval owners recorded in the Beads events table, including closed beads. Costs are per thread, with associations, and missing coverage remains visible.
